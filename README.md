...
This microservice manages the order catalog and provides information about the available orders.
...
- **Process management**: advanced orderion process management using [PM2](https://pm2.keymetrics.io)
...
# run docker container in orderion mode
yarn docker:prod
...
In orderion mode, only `info`, `warn`, and `error` logs will be printed to the console.\
It is up to the server (or process manager) to actually read them from the console and store them in log files.\
This app uses pm2 in orderion mode, which is already configured to store the logs in log files.
